...
namespace SysStore.Application.Customers.Orders
{
    ...
    {
        private readonly IUnitOfWork _unitOfWork;
        private OrderDetailValidator OrderDetailValidator { get; set; }
        ...
        {
            var employee = _unitOfWork.EmployeesRepository.FindFirstOrDefault(t => t.EmplId == employeId);
    ...
    public class OrderDetailValidator : AbstractValidator<OrderDetailRequest>
    {
        ...
        {
            _unitOfWork = unitOfWork;
            RuleFor(x => x.Unitprice).GreaterThan(0).WithMessage("Freight can't be empty");
            RuleFor(x => x.Qty).GreaterThan(0).WithMessage("Freight can't be empty");
            RuleFor(x => x.Productid).Must(ExistProduct).WithMessage("Employee does not exist");
        }
